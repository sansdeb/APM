...
// ─── STYLES — identical to AssetForm.jsx ─────────────────────
const S = {
...
// ─── MAIN COMPONENT ───────────────────────────────────────────
export default function ApproverFormView() {
  const { id }     = useParams();
...
useEffect(() => {
  ...
  const handleAction = async (action) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/api/approver/${id}/action`, { action, comments });
    ...
    <div style={{ minHeight: '100vh', background: '#f0f2f8', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <nav style={{ height: 52, background: 'linear-gradient(90deg,#1a3a8f,#1e4fc2)', display: 'flex', alignItems: 'center', padding: '0 24px', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>FOS &gt; Asset Page Management</span>
        </div>
        <button onClick={() => navigate('/dashboard/approver')}
      ...
      <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={S.form}>
          ...
          <div style={S.readOnlyBanner}>
            <span></span>
            <span>You are reviewing this request as an Approver. All fields are read-only.</span>
          </div>
          ...
          <div style={S.row}>
            ...
            <ReadField label="Status"       value={r.status?.status_description || r.status?.status_name} />
          </div>
            ...
            <table style={S.table}>
              ...
              <tbody>
                {(!r.items || r.items.length === 0) ? (
                  <tr>
                    <td colSpan={7} style={{ ...S.td, textAlign: 'center', color: '#aaa', padding: '20px 0' }}>
  ...
  <div style={S.totalsBox}>
    <div style={S.totalsFinalRow}>
      ...
      <span style={S.totalsFinalValue}>₹ {totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
          ...
          {/* ── Document Upload ──────────────────────────── */}
          <div style={S.sectionHeading}>Document Upload</div>
          ...
          <div style={S.btnRow}>
            ...
            <button disabled={submitting} onClick={() => handleAction('approve')}
              style={{ padding: '10px 24px', fontSize: 13, fontWeight: 600, border: 'none', borderRadius: 7, background: '#0c893a', color: '#fff', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}>
               Approve
            </button>
          </div>
